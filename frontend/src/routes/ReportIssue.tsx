...
export default function ReportIssue() {
        ...
        async function submit(e: React.FormEvent) {
                ...
                setBusy(true);
                try {
                        const r = await api.post("/complaints", fd, {
                                headers: {
                                        "Content-Type": "multipart/form-data",
                                },
                        });
                        if (r.data?.status === "duplicate_suspected") {
                                setSuggested(r.data.suggested || []);
                                setMessage(
                                        r.data.message ||
                                                "A similar complaint exists nearby."
                                );
                        } else if (r.data?.status === "created") {
                                setMessage(
                                        `Complaint created: ${r.data.complaint_id}`
                                );
                                setFiles([]);
                                setHashes([]);
                                setDesc("");
                                // nav(`/complaints/${r.data.complaint_id}`); // enable when detail route is live
                        } else {
        ...
        async function appendToExisting(id: string) {
                ...
                try {
                        ...
                        setMessage(`Update added to ${r.data.complaint_id}`);
                        setSuggested([]);
                        setFiles([]);
                        setHashes([]);
                } catch (err: any) {
